  ...
  function y() {
    ...
    function q() {
      ...
      if (d.getContext && d.getContext("2d") && (m = d.getContext("2d"), f = l.getContext("2d"),
        ...
        $(g).one("load", n);
        $(g).attr("src", "assets/login/img/bg_tiles_color.2.jpg")
      }
    }
  ...
  function z() {
    ...
    function n() {
      ...
      if (/login\?redirect/gi.test(window.location.href)) {
        var a =
          window.location.href.replace("login?", "signup?").replace("redirect", "signupRedirect"),
          a = a + "%3Fonboarding%26layouts";
        $(function() {
          if (1 === $("#signupLinkWrapper a").length) $("#signupLinkWrapper a").attr("href", a), $("#signupLinkWrapper a").on("mousedown", h);
          else {
        ...
        var d = window.location.href.replace("signup?",
          "login?").replace("signupRedirect", "redirect");
        $(function() {
          if (1 === $("#loginLinkWrapper a").length) $("#loginLinkWrapper a").attr("href", d), $("#loginLinkWrapper a").on("mousedown", h);
          else {
