...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><a href="<?= base_url('techadmin/media/categories')?>">Danh mục media</a></li>
...
<section class="content">
	<div class="row">
        <div class="col-md-12">
          <div class="box box-body">
            <div class="col-md-6">
              <h4><i class="fa fa-list"></i> &nbsp; Danh sách media</h4>
            </div>
            <div class="col-md-6 text-right">
              <div class="btn-group margin-bottom-20"> 
                <a href="<?= base_url('techadmin/media/add')?>" class="btn btn-success">
    ...
    <div class="box">
        ...
        <div class="box-body">
			...
			<form name="formbk" method="post" action="<?=base_url('techadmin/media/deletes')?>">
					...
					<tbody>
						...
						<tr>
							...
							<input type="number" onchange="update_sort($(this))" value="<?= @$v->sort ?>"
								data-placement='media'	id="sort"   data-item='<?= @$v->id; ?>' style="width: 55px">
							</td>
							...
							<td class="todo-list"><a href="<?= base_url('techadmin/media/edit/' . $v->id) ?>" title="Sửa sản phẩm">
                                <?= @$v->name ?>
                                </a>
								<div class="tools">
									<a href="<?=base_url('media-detail/'.$v->alias.'.html')?>" target="_blank">Xem trước</a>
									<a href="<?= base_url('techadmin/media/images/' . $v->id) ?>">Thêm ảnh</a>
									<a href="<?= base_url('techadmin/media/edit/' . $v->id) ?>">Chỉnh sửa</a>
									<a href="<?= base_url('techadmin/media/delete/' . $v->id) ?>" onclick="return confirm('Bạn có chắc chắn muốn xóa?')">Xóa</a>
								</div>
							</td>
							...
							<td class="text-center">
								<input class="tgl tgl-ios tgl_checkbox view_color" data-id="<?=$v->id;?>" id="cb_<?=$v->id;?>" <?=$v->active==1?'checked=""':''?> type="checkbox" data-value="<?=$v->id;?>" data-placement="media" data-view="active">
								<label class="tgl-btn " for="cb_<?=$v->id;?>" ></label>
							</td>
							<td class="text-center">
								<a class="btn btn-xs btn-default"
								   href="<?= base_url('techadmin/media/images/' . $v->id) ?>"><i class="fa fa-image"></i> </a>
								<a class="btn btn-xs btn-default"
								   href="<?= base_url('techadmin/media/edit/' . $v->id) ?>"><i
										class="fa fa-pencil"></i> </a>
								<a class="btn btn-xs btn-danger"
								   href="<?= base_url('techadmin/media/delete/' . $v->id) ?>" title="Xóa"
